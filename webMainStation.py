...
### STATION
    ...
    def index(self, error=''):
        with self.dbConnect() as dbConnection:
            self.engine.visits.checkBuilding(dbConnection)   #TODO - Move to thread
...
# strip whitespace before or after barcode digits (occasionally a space comes before or after)
        ...
        with self.dbConnect() as dbConnection:
            (current_keyholder_bc, _) = self.engine.keyholders.getActiveKeyholder(dbConnection)            
            for bc in barcodes:
                if (bc == KEYHOLDER_BARCODE) or (bc == current_keyholder_bc):
                    ...
                    if not current_keyholder_bc:
                        self.engine.keyholders.setActiveKeyholder(dbConnection, bc)
                    if error:
                        cherrypy.log(error)
    ...
    def keyholder(self, barcode):
        ...
        with self.dbConnect() as dbConnection:
            ...
            if (bc == KEYHOLDER_BARCODE) or (bc == current_keyholder_bc):            
                ...
                error = self.engine.keyholders.setActiveKeyholder(dbConnection, barcode)
